...
class FedAvgMom1(FedPartAvg):
    def __init__(self, aggregate_mode: str = "original", *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.global_optim_state = None
        self.fed_mom1_result = {}
        self.fed_mom1_model_results = {}
        self.aggregate_mode = aggregate_mode

    def get_results(self):
        return self.fed_mom1_result, self.fed_mom1_model_results, self.metrics_history
    ...
    def evaluate(
        ...
        if server_round in self.fed_mom1_model_results:  
            expand_fed_part_avg_model_results= {**self.fed_mom1_model_results[server_round], "global_loss": eval_res[0], "global_metrics": eval_res[1]}
        else:
            expand_fed_part_avg_model_results= {"global_loss": eval_res[0], "global_metrics": eval_res[1]}
        
        self.fed_mom1_model_results[server_round] = expand_fed_part_avg_model_results
    ...
    def aggregate_fit(
        ...
        if self.fed_mom1_result.get(server_round):
            self.fed_mom1_result[server_round]["total_size"] = total_size
        else:
            self.fed_mom1_result[server_round] = {"total_size": total_size}
    ...
    def _aggregate_optimizer_states_weighted(self, optimizer_states, similarities):
        ...
        for layer in range(len(optimizer_states[0]['state'])):
            ...
            optimizer_len = len(optimizer_states)
            for i in range(len(optimizer_states)):
                ...
                weighted_snd_momentum += optimizer_states[i]['state'][layer]['exp_avg_sq']
            weighted_snd_momentum = weighted_snd_momentum / optimizer_len
    ...
    def _cos_similarity_from_optimizer_states(self, optimizer_states, prev_global_optimizer_state):
        ...
        glob_state_vec = torch.cat([tensor.flatten() for tensor in fst_momentum_glob])
        ...
        for client in range(len(optimizer_states)):
            ...
            client_state_vec = torch.cat([tensor.flatten() for tensor in fst_momentum_client])
    ...
    def aggregate_evaluate(
        ...
        if self.fed_mom1_result.get(server_round):
            self.fed_mom1_result[server_round]["total_loss"] = total_loss
        else:
            self.fed_mom1_result[server_round] = {"total_loss": total_loss}
